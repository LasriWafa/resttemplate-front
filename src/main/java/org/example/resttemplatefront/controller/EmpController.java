package org.example.resttemplatefront.controller;


import org.example.resttemplatefront.domaine.EmpVo;
...
// HttpHeaders
        HttpHeaders headers = new HttpHeaders();
        headers.setAccept(List.of(MediaType.APPLICATION_JSON));
// Request to return JSON format Utiliser la classe RestTemplate de Spring 19
        headers.setContentType(MediaType.APPLICATION_JSON);
        HttpEntity<EmpVo> entity = new HttpEntity<EmpVo>(emp, headers);
        ResponseEntity<String> response = restTemplate.exchange(url, HttpMethod.POST, entity,
                String.class);
